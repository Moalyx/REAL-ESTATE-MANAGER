...
class AutocompleteRepositoryImpl @Inject constructor(
    ...
    override suspend fun getAutocompleteResult(address: String, localisation : String): List<PredictionAddressEntity> {
        ...
        val response : PredictionResponse = googleApi.autocompleteResult(BuildConfig.GOOGLE_AUTOCOMPLETE_KEY, localisation, "2", address)

        val addresses: List<Pair<String, String>?> = response.predictions.map { it ->
        ...
        return addresses.filterNotNull().map { PredictionAddressEntity(it.first, it.second ) }
